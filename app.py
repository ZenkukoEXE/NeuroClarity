...
def estimate_time(text):
    ...
    if any(word in text_lower for word in ['write', 'calculate', 'code', 'analyze', 'task']):
        base_minutes *= 2.0  # Harder sections
    elif any(word in text_lower for word in ['read', 'background', 'introduction', 'overview']):
        base_minutes *= 0.5  # Easier reading
        
    # Convert minutes to hours and round up
    base_hours = base_minutes / 60
    return round(base_hours, 2) if base_hours > 0 else 0.02  # Minimum 0.02 hours (1 minute)
...
def create_downloadable_docx(processed_sections):
    ...
    for section in processed_sections:
        ...
        doc.add_paragraph(f"⏱️ Suggested Time: {section['Time']} minutes").bold = True
...
if __name__ == "__main__":
    # Auto-launch Streamlit app
    os.system("streamlit run app")